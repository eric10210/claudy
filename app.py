...
# To use secrets: create .streamlit/secrets.toml with [bybit] api_key = "..." secret = "..."
default_api_key = st.secrets.get("bybit", {}).get("api_key", "") if hasattr(st, "secrets") else ""
default_secret   = st.secrets.get("bybit", {}).get("secret", "")  if hasattr(st, "secrets") else ""

api_key = st.sidebar.text_input("API Key", type="password", value=default_api_key, help="Create in Bybit account → API Management")
...
with st.spinner("Loading Market Data..."):
    try:
        if api_key and secret:
            ...
            # Set leverage for the selected symbol on Bybit
            try:
                exchange.set_leverage(leverage, symbol)
            except Exception:
                pass  # leverage may already be set or symbol uses default
                        ...
                        with st.status("Executing...", expanded=True) as status:
                            ...
                            try:
                                order = exchange.create_market_order(
                                    ...
                                    params={
                                        'leverage': leverage,
                                        'positionIdx': 0,  # 0 = one-way mode on Bybit
                                    },
                                )
            ...
            st.warning("⚠️ Enter your Bybit API Key & Secret in the sidebar to begin.")
            st.markdown("""
### How to connect
1. Log in to [Bybit](https://www.bybit.com) → **Account → API Management**
2. Create a key with **Read + Contract Trade** permissions (disable withdrawals)
3. Paste the key & secret in the sidebar

**Or** store them in `.streamlit/secrets.toml`:
```toml
[bybit]
